...
# -X drops uid/gid and the extra attribute field. That field carries the mtime
# as a UTC instant, so -X will also keep the zip independent of the packer's
# timezone. -D drops directory entries. The sorted file list fixes entry order.
(
    cd "$STAGING"
